...
# Importamos correctamente desde el módulo dentro del mismo paquete (homework)
...
def mapper_query_1(sequence):
    ...
    for index, (_, row) in enumerate(sequence):
        if index == 0:
            result.append((str(index), row.strip() + ",tip_rate"))
        else:
            row_values = row.strip().split(",")
            total_bill = float(row_values[0])
            tip = float(row_values[1])
            tip_rate = tip / total_bill
            result.append((str(index), row.strip() + "," + str(tip_rate)))
    return result


def reducer_query_1(sequence):
    """Reducer"""
    # Aseguramos que todas las claves sean strings para consistencia
    result = [(str(key) if not isinstance(key, str) else key, value) for key, value in sequence]
    return result
